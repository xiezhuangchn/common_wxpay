    ...
    if (this.refundCount != null && this.refundCount > 0) {
      ...
      for (int i = 0; i < this.refundCount; i++) {
        ...
        for (int j = 0; j < refundRecord.getCouponRefundCount(); j++) {
          WxPayRefundCouponInfo wxi = new WxPayRefundCouponInfo();
          wxi.setCouponType(this.getXmlValue("xml/coupon_type_" + i));
          wxi.setCouponRefundFee(this.getXmlValueAsInt("xml/coupon_refund_fee_" + i + "_" + j));
          wxi.setCouponRefundId(this.getXmlValue("xml/coupon_refund_id_" + i + "_" + j));
          coupons.add(wxi);
        }
